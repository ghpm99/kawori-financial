...
const TokenStep = ({ onSuccess, errorMessage, isLoading }: TokenStepProps) => {
    ...
    return (
        <>
            <Form form={form} layout="vertical" onFinish={(values) => onSuccess(values.token)} requiredMark={false}>
                ...
                <Form.Item name="token" rules={[{ required: true, message: "Digite o token recebido por e-mail" }]}>
                    <Input prefix={<KeyOutlined />} placeholder="Cole o token recebido por e-mail" size="large" />
                </Form.Item>
                <Form.Item>
